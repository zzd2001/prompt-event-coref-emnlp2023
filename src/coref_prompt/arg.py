...
def parse_args():
    ...
    parser.add_argument("--sample_strategy", default="corefnm", type=str, required=True, 
        help="The chosen sampling strategy.", choices=['no', 'random', 'corefnm', 'corefenn1', 'corefenn2']
    )
    # 负采样参数
    ...
    # ？？ action="store_true"：这指定了当--with_mask被命令行指定时，应该对应的动作。store_true表示如果命令行中出现了--with_mask，那么相应的变量（这里是args.with_mask）将被设置为True。默认情况下，如果不使用--with_mask选项，args.with_mask将为False。
